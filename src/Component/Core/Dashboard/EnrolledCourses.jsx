...
export default function EnrolledCourses() {
  ...
  useEffect(() => {
    setLoading(true);
    (async () => {
      console.log("Token used for enrolled courses API:", token)
      try {
        const res = await getUserEnrolledCourses(token)
        setEnrolledCourses(Array.isArray(res) ? res : [])
      } catch (error) {
        console.log("Could not fetch enrolled courses. Error:", error)
        setEnrolledCourses([])
      }
      setLoading(false);
    })()
  ...
  // Debug: log enrolledCourses every render
  console.log("enrolledCourses state:", enrolledCourses);
  return (
    <>
